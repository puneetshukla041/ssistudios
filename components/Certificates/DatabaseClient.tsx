...
const CertificateDatabaseClient: React.FC<Props> = ({ initialData }) => {
  // --- Global State ---
  const [refreshKey, setRefreshKey] = useState(0);
  const [certificateData, setCertificateData] = useState<ICertificateClient[]>(initialData?.data || []);
  const [totalRecords, setTotalRecords] = useState(initialData?.total || 0);
  const [isRefreshing, setIsRefreshing] = useState(false);
  const [uniqueHospitals, setUniqueHospitals] = useState<string[]>(initialData?.filters?.hospitals || []);
  ...
  // --- Stats State ---
  const [dbTotalRecords, setDbTotalRecords] = useState(totalRecords); 
  ...
  const [animatedTotalRecords, setAnimatedTotalRecords] = useState(dbTotalRecords);
  const [animatedHospitalCount, setAnimatedHospitalCount] = useState(uniqueHospitals.length);
  const [animatedDoctors, setAnimatedDoctors] = useState(doctorsCount);
  const [animatedStaff, setAnimatedStaff] = useState(staffCount);
    ...
    const fetchGlobalStats = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setDbTotalRecords(data.totalRecords || dbTotalRecords);
        }
      } catch (error) {
    ...
    <div className="min-h-screen bg-[#F8FAFC] text-slate-800 font-sans selection:bg-indigo-500/10 selection:text-indigo-700">
      ...
      <main className="mx-auto w-full max-w-[1600px] px-6 py-10 space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            {/* Other stat cards omitted for brevity; the component renders similarly to the original page */}

            </div>

            {/* ACTION TOOLBAR and CONTENT (reuse CertificateTable) */}
...
export default CertificateDatabaseClient;
