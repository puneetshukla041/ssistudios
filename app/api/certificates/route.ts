...
// Lightweight in-memory cache for short-lived identical queries
const CACHE_TTL_MS = 5000; // 5s
const CACHE_MAX_ENTRIES = 200;
const apiCache: Map<string, { expires: number; data: any }> = new Map();
...
export async function GET(req: NextRequest) {
    try {
        ...
        const cacheKey = `certificates:${searchParams.toString()}`;
        ...
        // Cache response for a short duration to avoid repeated DB load
        try {
            const oldestKey = apiCache.keys().next().value;
            if (apiCache.size >= CACHE_MAX_ENTRIES && oldestKey) {
                apiCache.delete(oldestKey);
            }
            apiCache.set(cacheKey, { expires: Date.now() + CACHE_TTL_MS, data: responsePayload });
        } catch (e) {
